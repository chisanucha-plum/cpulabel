...
class Config:
    ...
    classes: Dict[int, str] = field(
        default_factory=lambda: {0: "helmet", 1: "person", 2: "safety vest"}
    )
    class_colors: Dict[int, List[int]] = field(
...
class ConfigLoader:
    ...
    def get_class_threshold(
        classes_raw: Dict, class_id: int, threshold_type: str
    ) -> float:
        ...
        if isinstance(class_config, dict):
            ...
            return class_config.get(
                threshold_key, 0.45 if threshold_type == "box" else 0.30
            )
