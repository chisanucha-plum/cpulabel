...
class GroundingDINOModel:
    ...
    def _get_threshold(self, class_id: int, threshold_type: str) -> float:
        ...
        return (
            ...
            if threshold_type == "box"
            else self.config.text_threshold
        )
    ...
    def detect_multiclass(self, image, classes: dict) -> List[Detection]:
        ...
        for class_id, class_name in classes.items():
            class_key = str(class_id)
            class_config = self.classes_raw.get(class_key, {})
            if isinstance(class_config, dict) and class_config.get("detect") is False:
                continue
